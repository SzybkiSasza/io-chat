...
function logIn(socket, username) {
  ...
  if (user !== null) {
    if (user.socketIds.length >= MAX_USER_SOCKETS) {
...
function logOff(socket, username) {
  ...
  if (indexToDelete != -1) {
    if (users[indexToDelete].socketIds.length > 1) {
      removeSocket(socket);
...
function removeSocket(socketToRemove) {
  ...
  users.forEach(function(user, index) {
    user.socketIds.forEach(function(socketId, sindex) {
  ...
  if (userIndex != -1) {
    users[userIndex].socketIds.splice(socketIndex, 1);
...
function processMessage(socketFrom, data) {
  ...
  else { // If user doesn't exist - show him logon prompt
